...
@section('title', 'Edit Model - Qikleadz')
                        ...
                        <form method="POST" action="{{ route('brand.update', $modelDetail->id) }}">
                            ...
                            <div class="form-group">
                                ...
                                <select name="oem_id" id="oem_id" class="form-control">
                                    ...
                                    @foreach ($oems as $oem)
                                        <option value="{{ $oem->id }}"
                                            {{ $modelDetail->oem_id == $oem->id ? 'selected' : '' }}>
                                            {{ $oem->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="vehicle_type_id" id="vehicle_type_id" class="form-control" required>
                                    @foreach ($vehicleTypes as $vehicleType)
                                        <option value="{{ $vehicleType->id }}"
                                            {{ $modelDetail->vehicle_segment_id == $vehicleType->id ? 'selected' : '' }}>
                                            {{ $vehicleType->name }}
                                        </option>
                                    @endforeach
                                </select>
                                ...
                                <select name="industry_type_id" id="industry_type_id" class="form-control" required>
                                    @foreach ($industryTypes as $industryType)
                                        <option value="{{ $industryType->id }}"
                                            {{ $modelDetail->vehicle_usage_id == $industryType->id ? 'selected' : '' }}>
                                            {{ $industryType->name }}
                                        </option>
                                    @endforeach
                                </select>
