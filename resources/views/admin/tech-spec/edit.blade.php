            ...
            <div class="col-md-10">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{ route('tech-spec.update', $techSpec->id) }}">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="form-group mt-4">
                                ...
                                <select name="brand_id" id="brand_id" class="form-control" required
                                    ...
                                    @foreach ($brands as $brand)
                                        <option value="{{ $brand->id }}" {{ old('brand_id', $techSpec->brand_id) == $brand->id ? 'selected' : '' }}>
                                            {{ $brand->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="form-group mb-3">
                                ...
                                <select name="variant_id" id="variant_id" class="form-control" required>
                                    ...
                                    @foreach ($variants as $variant)
                                        <option value="{{ $variant->id }}" {{ old('variant_id', $techSpec->variant_id) == $variant->id ? 'selected' : '' }}>
                                            {{ $variant->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div id="feature-wrapper">
                                @foreach ($techSpecs as $f)
                                    <div class="feature-item mb-4 border p-3 rounded">
                                        <input type="hidden" name="techSpec_ids[]" value="{{ $f->id }}">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <textarea name="descriptions[]" class="form-control summernote-editor"
                                                rows="6">{{ old('descriptions.' . $loop->index, $f->description) }}</textarea>
                                        </div>

                                        {{-- Delete Existing Option --}}
                                        <div class="form-check mt-2">
                                            <input type="checkbox" name="delete_ids[]" value="{{ $f->id }}"
                                                class="form-check-input" id="delete_{{ $f->id }}">
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#add-feature').on('click', function () {
                let newFeature = `
                    <div class="feature-item mb-4 border p-3 rounded">
                        <input type="hidden" name="techSpec_ids[]" value="">
                        <div class="form-group">
                            <label>Title</label>
