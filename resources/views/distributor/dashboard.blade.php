                                    ...
                                    <table class="table table-hover text-nowrap">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    <span class="badge badge-{{ $lead->status == 'converted' ? 'success' : ($lead->status == 'new' ? 'info' : ($lead->status == 'pending' ? 'warning' : 'secondary')) }}">
                                                        {{ ucfirst($lead->status) }}
                                                    </span>
                                                </td>
                                                <td>{{ $lead->created_at->format('d M Y') }}</td>
                                                <td>
                                                    <a href="{{ route('distributor.leads.show', $lead->id) }}" class="btn btn-sm btn-info">
                                            ...
                                            <div class="card card-primary card-outline">
                                                <div class="card-body box-profile">
                                                    <div class="text-center">
                                                        @if($dealer->logo)
                                                            <img class="profile-user-img img-fluid img-circle" src="{{ Storage::url($dealer->logo) }}" alt="Dealer logo">
                                                        @else
                                                            <div class="profile-user-img img-fluid img-circle bg-primary d-flex align-items-center justify-content-center">
                                                                <span class="text-white" style="font-size: 24px;">{{ substr($dealer->name, 0, 1) }}</span>
                                                            </div>
                                                        @endif
                                                    </div>
                                                    ...
                                                    <ul class="list-group list-group-unbordered mb-3">
                                                        ...
                                                        <li class="list-group-item">
                                                            <b>Phone</b> <a class="float-right">{{ $dealer->mobile ?? 'N/A' }}</a>
                                                        </li>
                                                        <li class="list-group-item">
                                                            <b>Status</b>
                                                            <span class="float-right badge badge-{{ $dealer->status == 'Active' ? 'success' : 'danger' }}">
                                                                {{ $dealer->status }}
                                                            </span>
                                                        </li>
                                                    </ul>
